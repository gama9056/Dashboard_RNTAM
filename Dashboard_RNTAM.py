...
# Base de datos maestra de deforestación por minería ilegal (Hectáreas)
datos_deforestacion = {
...
# ==========================================================
# LÓGICA INTERACTIVA DE ENFOQUE Y FILTRADO ANALÍTICO
# ==========================================================
if pvc_seleccionados and not gdf_ambitos.empty:
    ...
    # 2. Datos analíticos reactivos al filtro
    ...
    # 3. Filtrar el diccionario para el gráfico del panel derecho
    datos_grafico = {p: datos_deforestacion.get(p, 0.0) for p in pvc_seleccionados}
    
    # 4. Redacción reactiva del reporte situacional
    sectores_texto = ", ".join(pvc_seleccionados)
    reporte_dinamico = f"""
    El análisis geoespacial enfocado en los sectores de {sectores_texto} muestra un escenario crítico directamente vinculado a actividades de minería aurífera ilegal. La cuantificación detallada en estas zonas específicas revela un impacto directo sobre la cobertura boscosa que altera los ecosistemas protegidos dentro del área de influencia analizada.
    ...
    # Configuración por defecto (Toda la Reserva)
    ...
    # El gráfico muestra todos los sectores ordenados
    datos_grafico = datos_deforestacion.copy()
...
with col_center:
    ...
    folium.TileLayer(tiles=url_satelite, attr="Google Maps Satellite", name="Google Satélite", overlay=False, control=True).add_to(m)

    if not gdf_za.empty:
        folium.GeoJson(gdf_za, name="🔶 Zona de Amortiguamiento (ZA)", style_function=lambda x: {'fillColor': '#e67e22', 'color': '#d35400', 'weight': 1.5, 'fillOpacity': 0.15}).add_to(m)

    if not gdf_filtrado.empty:
        folium.GeoJson(gdf_filtrado, name="📂 Ámbitos de Control PVC", style_function=lambda x: {'fillColor': '#8b0000', 'color': '#5c0000', 'weight': 1.5, 'fillOpacity': opacidad}, tooltip=folium.GeoJsonTooltip(fields=["NOM_PVC"], aliases=["Ámbito de Control: "])).add_to(m)
    ...
    if not gdf_pvc_filtrado.empty:
        ...
        for idx, row in gdf_pvc_filtrado.iterrows():
            if row.geometry and row.geometry.geom_type == 'Point':
                ...
                folium.Marker(location=coords, popup=f"<b>Puesto de Vigilancia y Control:</b><br>{nombre_puesto}", tooltip=f"🏠 {nombre_puesto}", icon=folium.Icon(color="darkblue", icon="shield", prefix="fa")).add_to(fg_puestos)
        fg_puestos.add_to(m)
...
# ==========================================================
# PANEL DERECHO (ANALÍTICA CRÍTICA DE DEFORESTACIÓN REACTIVA)
# ==========================================================
with col_right:
    with st.container(height=600, border=True):
        st.markdown("<h3 style='text-align:center; color:#163b16; margin-top:0;'>📉 ANÁLISIS DE PÉRDIDA BOSCOSA</h3>", unsafe_allow_html=True)
        
        # El DataFrame ahora se arma única y exclusivamente con los datos filtrados en la sesión
        ...
        # El gráfico se reduce o se expande dinámicamente según la selección
        ...
        # El texto se escribe con los párrafos dinámicos construidos arriba
